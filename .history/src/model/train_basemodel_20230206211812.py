...
def wconvert_val_data(prefix: str, wildcard: str) -> None:
    ...
    for tfrec_file in all_files:
        for example in tf.python_io.tf_record_iterator(tfrec_file):
            ...
            seg_start = list(
                tf_example.features.feature['segment_start_times'].int64_list.value)
            seg_labels = list(
                tf_example.features.feature['segment_labels'].int64_list.value)
            seg_scores = list(
                tf_example.features.feature['segment_scores'].float_list.value)

            tf_seq_example = tf.train.SequenceExample.FromString(example)
            num_frames = len(
                tf_seq_example.feature_lists.feature_list['audio'].feature)

            # why are there videos with invalid labels?
            if any(np.array(seg_start) > num_frames):
...
def generate_submission(ids: np.array, probas: np.array) -> None:
    ...
    with open('submission.csv', 'w') as fout:
        ...
        for i in range(NUM_CLASSES):
            ...
            fout.write('{},{}\n'.format(
                classes_table[i], ' '.join(current_line)))
...
if __name__ == '__main__':
    ...
    convert_val_data(
        'val', '/kaggle/input/youtube-challenge-2019-val/val/validate*.tfrecord')
    test_loader = load_test_data(
        '/kaggle/input/youtubechallenge2019test/test/test*.tfrecord')
