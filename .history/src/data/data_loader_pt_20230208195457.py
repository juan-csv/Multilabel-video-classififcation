...
# get the number of samples from dataset_all
N_SAMPLES = 


counter = tf.data.Dataset.from_tensors(0).repeat()
dataset = tf.data.TFRecordDataset(list_train_files).zip((counter, dataset)).map(lambda x, example: (x[0] + 1, example)).prefetch(tf.data.AUTOTUNE)
dataset_length = int(tf.data.Dataset.from_tensors(0).repeat().reduce(0, lambda x, _: x + 1).numpy())
