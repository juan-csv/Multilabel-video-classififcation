...
dataset = tf.data.TFRecordDataset(list_train_files , num_parallel_calls=tf.data.AUTOTUNE)
dataset = tf.data.TFRecordDataset(filenames, num_parallel_calls=tf.data.AUTOTUNE)
...
class TFRecordDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        features = tf.io.decode_jpeg(example['features'])
        features = tf.transpose(features, [2, 0, 1])  # HWC -> CHW
        return features, example['labels']
