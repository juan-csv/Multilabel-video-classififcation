...
class YoutubeVideoDataset(YoutubeSegmentDataset):
    def prepare_one_sample(self, row):
        ...
        # Frames. Shape: (frames, 1024)
        frames = context.feature['mean_rgb'].float_list.value
        #tmp = features.feature_list['rgb'].feature

        frames = f_bytes2array(tmp)

        # Audio. Shape: (frames, 128)
        audio = context.feature['mean_audio'].float_list.value
        #tmp = features.feature_list['audio'].feature
        audio = f_bytes2array(tmp)
...
filepaths = glob.glob( train_pattern_files.__str__() )

print(filepaths[:2])
dataset = YoutubeVideoDataset(filepaths)
