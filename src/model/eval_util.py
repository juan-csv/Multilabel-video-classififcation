...
if __name__=="__main__":
    ...
    # create dummy input
    actuals = np.random.randint(0, 2, size=(BS, N_CLASSES))
    # random matrix with values between 0 to 1
    predictions = np.random.rand(BS, N_CLASSES)
    
    # test metrics
    gap = calculate_gap(predictions, actuals, top_k=20)
    hit_at_one =calculate_hit_at_one(predictions, actuals)
    perr = calculate_precision_at_equal_recall_rate(predictions, actuals)
    
    print("gap", gap)
    print("hit_at_one", hit_at_one)
    print("perr", perr)